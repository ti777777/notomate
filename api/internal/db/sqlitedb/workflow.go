...
func (s SqliteDB) CreateWorkflow(w model.Workflow) error {
	return gorm.G[model.Workflow](s.getDB()).Create(context.Background(), &w)
}

func (s SqliteDB) FindWorkflow(w model.Workflow) (model.Workflow, error) {
	return gorm.G[model.Workflow](s.getDB()).
		Where("id = ?", w.ID).
		Take(context.Background())
}

func (s SqliteDB) FindWorkflows(f model.WorkflowFilter) ([]model.Workflow, error) {
...
func (s SqliteDB) UpdateWorkflow(w model.Workflow) error {
...
func (s SqliteDB) UpdateWorkflowEnabled(id string, enabled bool, updatedAt, updatedBy string) error {
	return s.getDB().
...
func (s SqliteDB) CreateWorkflowRun(r model.WorkflowRun) error {
	return gorm.G[model.WorkflowRun](s.getDB()).Create(context.Background(), &r)
}

func (s SqliteDB) FindWorkflowRun(r model.WorkflowRun) (model.WorkflowRun, error) {
	return gorm.G[model.WorkflowRun](s.getDB()).
		Where("id = ?", r.ID).
		Take(context.Background())
}

func (s SqliteDB) FindWorkflowRuns(f model.WorkflowRunFilter) ([]model.WorkflowRun, error) {
...
func (s SqliteDB) UpdateWorkflowRunStatus(id string, status string, startedAt, finishedAt string) error {
...
func (s SqliteDB) NextWorkflowRunNumber(workflowID string) (int, error) {
	var next int
	err := s.getDB().
...
func (s SqliteDB) DeleteWorkflowRunsBefore(createdBefore string) error {
	db := s.getDB()
...
func (s SqliteDB) CreateWorkflowJob(j model.WorkflowJob) error {
	return gorm.G[model.WorkflowJob](s.getDB()).Create(context.Background(), &j)
}

func (s SqliteDB) FindWorkflowJob(j model.WorkflowJob) (model.WorkflowJob, error) {
	return gorm.G[model.WorkflowJob](s.getDB()).
		Where("id = ?", j.ID).
		Take(context.Background())
}

func (s SqliteDB) FindWorkflowJobs(f model.WorkflowJobFilter) ([]model.WorkflowJob, error) {
...
func (s SqliteDB) UpdateWorkflowJobStatus(id string, status string, startedAt, finishedAt string) error {
	updates := map[string]interface{}{"status": status}
	if startedAt != "" {
		updates["started_at"] = startedAt
	}
...
func (s SqliteDB) ClaimQueuedWorkflowJob(runnerID string, labels []string) (model.WorkflowJob, error) {
	return claimQueuedWorkflowJob(s.getDB(), runnerID, labels)
}

func (s SqliteDB) AppendWorkflowJobLogs(jobID string, startLine int, lines []string, createdAt string) error {
	return appendWorkflowJobLogs(s.getDB(), jobID, startLine, lines, createdAt)
}

func (s SqliteDB) FindWorkflowJobLogs(jobID string, afterLine int, limit int) ([]model.WorkflowJobLog, error) {
...
func (s SqliteDB) CountWorkflowJobLogs(jobID string) (int, error) {
	var count int64
	err := s.getDB().
